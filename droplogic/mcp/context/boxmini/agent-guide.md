...
- For linear `reservoir_extraction`, pass `linear_space_per_row=2`, `linear_space_per_col=2`, and `linear_vital_space=2` for `2 x 2` or larger droplets unless the user gives another spacing. The library also infers these safer defaults for larger droplets, but explicit arguments make the plan easier to audit.
