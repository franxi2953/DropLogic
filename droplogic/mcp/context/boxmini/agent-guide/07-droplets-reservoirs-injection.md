...
- Except for injection/loading regions and explicit waste/trash routing, when geometry, `vital_space`, and routing corridors allow, try to place outer droplets within about `5` electrodes of the chip sides while still avoiding the exact border electrodes.
- Passing target validation only proves the final footprint is legal; SIPP can still fail when a target is tight against an edge or leaves poor approach corridors. For larger footprints or larger `vital_space`, keep extra margin beyond the validated footprint/vital space, and use nearer/intermediate targets after one failed edge-side target attempt. If an edge-near layout is requested, reach those targets through staged/intermediate moves when needed.
