...
- Persisted droplet captures use `capture_source="pause_streamer"`: they stop the live streamer, acquire directly from the microscope, then restore the live view and low-light state. The streamer is for viewing only, not experimental image acquisition.
