...
- Normal BoxMini execution should remain in microscope brightfield with droplet-follow tracking. Do not switch to `whole_chip_camera` just because an overview could be informative; use it only when the user explicitly asks for whole-chip/cartridge viewing or a fixed-view segment is clearly required.
