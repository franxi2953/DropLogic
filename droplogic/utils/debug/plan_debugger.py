...
class _FrameVisualizer(QWidget):
    ...
    def _set_frame(self, frame_matrix: np.ndarray, droplet_positions: Dict[int, Tuple[int, int]], 
...
class _VisualFrameExplorer(QMainWindow):
    ...
    def _init_ui(self):
        ...
        for frame_idx, desc in self.all_events:
            ...
            action.triggered.connect(lambda checked, f=frame_idx: self._set_frame(f))
        ...
        all_conflicts_btn.clicked.connect(self._show_all_conflicts)
        ...
        self.droplet_search.textChanged.connect(self._update_display)
        search_layout.addWidget(self.droplet_search)

        self.droplet_selector = QComboBox()
        self.droplet_selector.setMinimumWidth(80)
        self.droplet_selector.currentIndexChanged.connect(self._on_droplet_selected)
        ...
        self.visualizer.cursorPositionChanged.connect(self._update_cursor_position)
    ...
    def _on_droplet_selected(self, idx):
    ...
    def _get_droplet_color(self, droplet_id: int) -> str:
    ...
    def _update_display(self):
        ...
        for d_id in filtered_ids:
            ...
            color = self._get_droplet_color(d_id)
        ...
        self.visualizer._set_frame(frame_matrix, droplet_positions, active_droplets, self.droplets, frame_events)
...
def open_explorer(pickle_file_path: str = None):
    ...
    window = _VisualFrameExplorer(pickle_file_path)
    window.show()
    sys.exit(app.exec_())
