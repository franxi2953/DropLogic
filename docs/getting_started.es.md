...
Para detalles sobre el archivo de estado por defecto, que necesita cada sistema y como mantener calibraciones especificas de maquina en local, consulta [Configuracion](configuration.md).
...
- `examples/DMLite_example.py`: Se trata exactamente del mismo código de enrutamiento, pero conectado al hardware AM-EWOD `DMLite` de Acxel mediante la capa adaptadora de DropLogic. Implementa un `frame_delay` mayor para respetar las demoras de actuación de voltaje y la física de los fluidos.
